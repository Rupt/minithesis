...
# If CHECKOUT_PATH contains a previous version of your thesis, substitutue it
# below.
# Building the diff is easiest if you first clean the repository, so you can
# use commands like:
# make clean && make main.hard.diff.pdf
# to reliably build your diff.
#
# Removed because non-portable.
# main.hard.diff.pdf: main.hard.diff.tex main.hard.bbl
#
# main.hard.diff.tex: main.hard.tex ALWAYS_REBUILD
# 	latexdiff --flatten --ignore-warnings CHECKOUT_PATH/main.hard.tex main.hard.tex > main.hard.diff.tex 2> /dev/null
#
...
.PHONY: clean
clean:
	rm -rf scratch *.pdf main.hard.diff.tex main.hard.bbl
